        ...
        <v-col cols="12">
          <v-card tile>
            ...
            <v-card-text class="pa-0">
              <v-data-table :loading="loadingItems" :headers="headers" :items="items" item-key="$index">
...
export default {
  ...
  data() {
    return {
      ...
      headers: [
        ...
        {
          text: this.t('UI.Column.PropertyName'),
          value: 'propertyName',
        },
        {
          text: this.t('UI.Column.UserName'),
          value: 'userName',
        },
        {
          text: this.t('UI.Column.Value'),
          value: 'value',
        },
        {
          text: 'Action',
  ...
  computed: {
    ...mapGetters(['PropertyDefine', 'Users']),
  },
  methods: {
    resetFilter() {
    ...
    async fetchRecords() {
      ...
      const { data } = await this.$store.dispatch('fetchConfigurations')
      for (let c of data) {
        const p = this.PropertyDefine.find((s) => s.value === c.propertyId)
        c.propertyName = p ? p.text : ''
        const u = this.Users.find((s) => s.value === c.userId)
        c.userName = u ? u.text : ''
      }
    ...
    handleImport() {},
    handleViewItem() {},
    handleEditItem({ name, propertyName }) {
      const data = { key: name, name: propertyName }
      this.$router.push({
        path: `/sys/config/item/${JSON.stringify(data)}`,
      })
    },
    handleSubmit() {},
    handleRefreshItem() {
